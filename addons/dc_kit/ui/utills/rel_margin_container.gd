...
var margin_scale_left: float = 0.0: set = _set_left
@export_range(-1.0, 100.0, 0.1, "or_greater")
var margin_scale_right: float = 0.0: set = _set_right
@export_range(-1.0, 100.0, 0.1, "or_greater")
var margin_scale_top: float = 0.0: set = _set_top
@export_range(-1.0, 100.0, 0.1, "or_greater")
var margin_scale_bottom: float = 0.0: set = _set_bottom

var _base_values: Dictionary = {}
var _in_theme_update := false  # reentrancy guard

func _set_left(v: float) -> void:
	if is_equal_approx(v, margin_scale_left): return
	margin_scale_left = v
	_apply_side(MarginSide.LEFT, v)

func _set_right(v: float) -> void:
	if is_equal_approx(v, margin_scale_right): return
	margin_scale_right = v
	_apply_side(MarginSide.RIGHT, v)

func _set_top(v: float) -> void:
	if is_equal_approx(v, margin_scale_top): return
	margin_scale_top = v
	_apply_side(MarginSide.TOP, v)

func _set_bottom(v: float) -> void:
	if is_equal_approx(v, margin_scale_bottom): return

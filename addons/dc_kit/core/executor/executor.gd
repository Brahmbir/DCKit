...
func get_command_def_array() -> Array[DCDefinition]:
	...
	def_arr.append(DCDefinition.new(
		"clear",
		...
		func(ctx: DCContext) -> DCResult:
			if ctx.args_length() != 1:
				return DCResult.fail(
                    "profile requires one argument (nested command)."
				)
			if profiler == null:
...
func _execute(node: _DCKitNamespace.Parser.CommandNode, exec: _Execution, kind: int) -> DCResult:
	...
	cmd_ctx._bind_raw_runner(func(s: String) -> DCResult:
	...
	if is_root: _active_ctx = cmd_ctx
	...
	if is_root: _active_ctx = null
	
	var result := _normalise(raw_result)
	
	# Don't spam the console with successful utility commands.
	match definition.log_mode:
		DCDefinition.LogMode.NORMAL:_log_result(result, exec.origin, exec.call_stack.snapshot())
		DCDefinition.LogMode.HIDDEN_ALL:pass
...
func _resolve_ctor(node: _DCKitNamespace.Parser.ConstructorNode, exec: _Execution, root_ctx: DCContext) -> DCResult:
	...
	if ctor_result is DCResult:
		if not ctor_result.success: return ctor_result
...
func _normalise(raw) -> DCResult:
	if raw is DCResult: return raw
	if raw == null:     return DCResult.ok(null)
...
class _CallStack:
	enum FrameKind { EXECUTION, ARGUMENT }
	...
	func depth()           -> int: return frames.size()
	func execution_depth() -> int: return _execution_depth
	func argument_depth()  -> int: return _argument_depth
...
class _Execution:
	...
	func _init(p_raw: String, p_origin: String, p_root: DCContext, p_stack: _CallStack) -> void:
		raw = p_raw; origin = p_origin; abort_root = p_root; call_stack = p_stack

	func record(r: DCResult)   -> void:     results.append(r)
	func to_result()           -> DCResult: return results.back() if not results.is_empty() else DCResult.ok(null)
#endregion

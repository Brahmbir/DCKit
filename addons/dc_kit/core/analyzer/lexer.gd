...
func _read_variable() -> Dictionary:
	...
	if _peek() == "{":
		return _read_braced_variable(start, is_silent)

	return _read_compact_variable(start, is_silent)


func _read_braced_variable(start: int, is_silent: bool) -> Dictionary:
	_eat()  # '{'
	_skip_whitespace()
	...
	var tok := Token.new(TokenType.VARIABLE, name, start, is_silent)

	_skip_whitespace()
	if not _at_end() and _peek() == ":":
		if not is_silent:
			return _err("Fallbacks are only allowed for silent variables.", _pos)
		_eat()
		var fb := _read_fallback(true)
		if not fb.ok: return fb
		tok.fallback = fb.token

	_skip_whitespace()
	if _at_end() or _peek() != "}":
		return _err("Unclosed '${...}'.", start)
	_eat()

	return { ok = true, token = tok }


func _read_compact_variable(start: int, is_silent: bool) -> Dictionary:
	if _peek() == "?":
		_eat()
		is_silent = true

	if not _is_ident_start(_peek()):
		return _err("Invalid char after '$%s'." % ("?" if is_silent else ""), start)
	...
	if is_silent and not _at_end() and _peek() == ":":
		_eat()
		var fb := _read_fallback(false)
		if not fb.ok: return fb
		tok.fallback = fb.token

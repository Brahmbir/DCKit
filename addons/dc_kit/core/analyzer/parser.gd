...
class StringNode:
	...
	func _init(p_value: String, p_quoted := false, p_start: int = -1, p_end: int = -1) -> void:
		value = p_value; quoted = p_quoted; start_pos = p_start
...
class ConstructorNode:
	...
	func _init(p_type: String, p_start: int) -> void:
		type_name = p_type; start_pos = p_start
...
class VariableNode:
	...
	func _init(p_name: String, p_silent := false, p_start: int = -1) -> void:
		name = p_name; is_silent = p_silent; start_pos = p_start
...
func _parse_arg() -> Dictionary:
	...
	match t.type:
		TT.VARIABLE:
			_advance(); return { ok = true, node = _make_var_node(t) }
		TT.LPAREN:
		...
		TT.IDENTIFIER:
			...
			if "." in t.value:
				return _fail("'%s': dotted names are only valid as command names." % t.value, t.position)
			_advance()
			return { ok = true, node = StringNode.new(t.value, false, t.position) }
		TT.NUMBER:
			_advance(); return { ok = true, node = StringNode.new(t.value, false, t.position) }
		TT.STRING:
			_advance(); return { ok = true, node = StringNode.new(t.value, true, t.position) }
		TT.RPAREN:
...
func _parse_constructor() -> Dictionary:
	var name_t := _peek(); _advance()
	if "." in name_t.value:
		return _fail("Constructor type '%s' cannot be a dotted name." % name_t.value, name_t.position)
	var open_t := _peek(); _advance()
	var ctor   := ConstructorNode.new(name_t.value, name_t.position)

	if not _at_end() and _peek().type == TT.RPAREN:
		_advance(); ctor.end_pos = _tokens[_pos - 1].position + 1
	...
	while not _at_end() and _peek().type != TT.RPAREN:
		...
		if _peek().type == TT.COMMA:        _advance()
...
func _parse_constructor_part(owner: String) -> Dictionary:
	...
	match t.type:
		TT.VARIABLE:
			if _peek_next().type == TT.LPAREN and _is_adjacent(t, _peek_next()):
				return _fail("Dynamic constructor type '$%s(...)' is not allowed." % t.value, t.position)
			_advance(); return { ok = true, node = _make_var_node(t) }
		TT.IDENTIFIER:
			if _peek_next().type == TT.LPAREN and _is_adjacent(t, _peek_next()):
				return _parse_constructor()
			if "." in t.value:
				return _fail("'%s': dotted names are not valid inside constructors." % t.value, t.position)
			_advance()
			return { ok = true, node = StringNode.new(t.value, false, t.position) }
		TT.NUMBER, TT.STRING:
			_advance()
...
func _parse_nested_command() -> Dictionary:
	...
	if name_t.type != TT.IDENTIFIER:
		return _fail("Expected command name inside '(...)', got '%s'." % name_t.value, name_t.position)
	...
	_advance(); cmd.end_pos = _tokens[_pos - 1].position + 1
	return { ok = true, node = cmd }
...
func _token_to_node(token) -> Object:
	...
	match token.type:
		TT.VARIABLE: return _make_var_node(token)
		TT.STRING:   return StringNode.new(token.value, true,  token.position)
		_:           return StringNode.new(token.value, false, token.position)

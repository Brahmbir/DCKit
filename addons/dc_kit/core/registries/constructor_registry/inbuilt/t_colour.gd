	...
	var handler := func(parts: Array) -> Variant:
		...
		if parts.size() == 1:
			...
			return DCResult.fail(
				...
				% s + "Available names: red, green, blue, yellow, cyan, magenta, "
		...
		if parts.size() in [3, 4]:
			...
			var all_whole : bool = values.all(func(v: float) -> bool: return is_equal_approx(v, floorf(v)))
			var any_gt1 : bool = values.any(func(v: float) -> bool: return v > 1.0)
		...
		return DCResult.fail(
			"Color: expected 0, 1, 3, or 4 parts — got %d." % parts.size())

...
static func create() ->  _DCKitNamespace.ConstructorDef:
	...
	var handler := func(parts: Array) -> Variant:
		...
		return DCResult.fail(
			"Vector3i: expected 0, 1, or 3 parts — got %d." % parts.size())
		...
		[
			sig_zero("Zero vector — Vector3i(0, 0, 0)"),
			 ...
			 _DCKitNamespace.ConstructorDef.TypeSignature.new(
				...
				[part_hint_vector3i(
						"value",
						"Existing Vector3i or integer-valued Vector3")]
			),
...
static func extract_vector3i(val: DCResult.Value) -> Variant:
	var r := val.raw
	if r is Vector3i: return r
	if r is Vector3: return Vector3i(r)
	return null
 
static func part_hint_vector3i(p_name: String, p_desc: String = "") ->  _DCKitNamespace.ConstructorDef.PartHint:
	return ( _DCKitNamespace.ConstructorDef.PartHint
		.new(p_name, "<Vector3i>", p_desc)
		.accepts([TYPE_VECTOR3I, TYPE_VECTOR3]))

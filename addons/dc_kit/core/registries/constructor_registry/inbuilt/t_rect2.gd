	...
	var handler := func(parts: Array) -> Variant:
		...
		if parts.size() == 1:
			...
			return DCResult.fail(
				"Rect2: single-part form expects a Rect2 or Rect2i, got %s."
				% val_label(val))
		...
		return DCResult.fail(
			"Rect2: expected 0, 1, 2, or 4 parts — got %d." % parts.size())
...
static func extract_rect2(val: DCResult.Value) -> Variant:
	...
	if r is Rect2i: return Rect2(r)   # widening — always lossless
	return null
